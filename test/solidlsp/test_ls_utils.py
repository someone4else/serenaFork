...
class TestReadFile:
    ...
    def test_legacy_encoding_is_detected_and_cached(self, tmp_path, caplog):
        # cp1251 text is not valid UTF-8, so the encoding has to be detected.
        p = tmp_path / "legacy.txt"
        p.write_bytes("привет мир, это текст на русском языке\n".encode("cp1251"))
        with caplog.at_level(logging.DEBUG, logger="solidlsp.ls_utils"):
            first = FileUtils.read_file(str(p), "utf-8")
            second = FileUtils.read_file(str(p), "utf-8")
        assert first == second
        assert "привет" in first
        # The detection is reported once per (path, mtime), not on every read.
        assert len([r for r in caplog.records if "using best match" in r.message]) == 1
