...
class Project(ToStringMixin):
    ...
    def _gather_ignorespec(self) -> None:
        with LogTime(f"Gathering ignore spec for project {self.project_config.project_name}", logger=log):
            ...
            # Gather ignored paths from .gitignore files first, then from the global and project
            # configuration. Order matters: pathspec uses "last match wins", so the explicitly
